...
/**
 * TransactPay — Maltese EMI specialising in BIN sponsorship.
 *
 * License:  MFSA (Malta Financial Services Authority), EMI
 *           + Gibraltar Financial Services Commission, EMI
 * Scheme:   Visa + Mastercard (principal member of both)
 * Coverage: All EU/EEA (Malta licence) + UK (Gibraltar licence)
 * Services: BIN sponsorship, card issuing (debit/credit/prepaid),
 *           virtual + physical + tokenized cards, e-wallets
 *
 * TransactPay is the go-to BIN sponsor for fintechs that don't hold
 * their own EMI licence. Modular card programmes via REST API.
 *
 * @see https://transactpay.com/
 */
class TransactPayCardIssuer implements CardIssuerInterface
{
    ...
    public function createVirtualCard(string $userId, string $cardholderName, string $currency = 'EUR'): array
    {
        $holder = $this->request('POST', '/api/v1/cardholders', [
            'name' => $cardholderName, 'external_id' => $userId, 'programme' => $this->programmeToken,
        ]);
        ...
        return [
            ...
            'scheme' => strtoupper($card['scheme'] ?? 'MASTERCARD'),
        ];
    }

    public function activateCard(string $cardId): array
    {
        $this->request('POST', "/api/v1/cards/{$cardId}/activate");
        return ['cardId' => $cardId, 'status' => 'ACTIVE'];
    }
    public function blockCard(string $cardId): array
    {
        $this->request('POST', "/api/v1/cards/{$cardId}/suspend");
    ...
    {
        $this->request('POST', "/api/v1/cards/{$cardId}/terminate");
    ...
    public function getCard(string $cardId): array
    {
        ...
        return [
            ...
            'expiryYear' => (int) ($c['expiry_year'] ?? 0), 'scheme' => strtoupper($c['scheme'] ?? 'MASTERCARD'),
        ];
    }
    public function provisionDigitalCard(string $cardId, string $deviceId, string $deviceFingerprint): array
    {
        $r = $this->request('POST', "/api/v1/cards/{$cardId}/tokenize", [
            'device_id' => $deviceId, 'device_fingerprint' => $deviceFingerprint, 'wallet_type' => 'HCE',
        ]);
        return [
    ...
    private function request(string $method, string $path, array $body = []): array
    {
        ...
        try { return $this->httpClient->request($method, $this->baseUrl . $path, $opts)->toArray();
            ...
            $this->logger->error('TransactPay API error', ['path' => $path, 'error' => $e->getMessage()]);
            throw new OpenBankingException("TransactPay error: {$e->getMessage()}", (int) $e->getCode(), $e);
        }
    }
}
