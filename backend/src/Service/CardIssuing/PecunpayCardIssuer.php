...
/**
 * Pecunpay (Pecunia Cards EDE) — Spanish EMI card issuer.
 *
 * License:  Bank of Spain (Banco de España), EMI — CSB 6707
 * Scheme:   Visa + Mastercard + China UnionPay (first Spanish UPI issuer)
 * Coverage: All SEPA zone (passported from Spain)
 * Powers:   Pagaqui (Portugal), Spanish fintechs, 500K+ cards managed
 * Services: White-label card issuing, BIN sponsorship, payment terminals
 *
 * Pecunpay specialises in white-label card programmes for fintechs.
 * API-first approach with IBM Red Hat OpenShift infrastructure.
 *
 * @see https://www.pecunpay.es/
 */
class PecunpayCardIssuer implements CardIssuerInterface
{
    ...
    {
        $holder = $this->request('POST', '/api/v2/cardholders', [
            'programme_id' => $this->programmeId, 'name' => $cardholderName,
            'external_id' => $userId, 'currency' => $currency,
        ]);
        $card = $this->request('POST', '/api/v2/cards', [
            'cardholder_id' => $holder['id'], 'type' => 'VIRTUAL', 'currency' => $currency,
        ]);
        return [
    ...
    public function getCard(string $cardId): array
    {
        ...
        return [
            ...
            'expiryYear' => (int) ($c['expiry_year'] ?? 0),
    ...
    {
        $r = $this->request('POST', "/api/v2/cards/{$cardId}/digitalize", [
    ...
    {
        $r = $this->request('POST', "/api/v2/tokens/{$tokenReferenceId}/session", ['atc' => $currentAtc + 1]);
        return [
    ...
    {
        $this->request('DELETE', "/api/v2/tokens/{$tokenReferenceId}");
    ...
    {
        $r = $this->request('GET', "/api/v2/tokens/{$tokenReferenceId}");
    ...
    {
        $r = $this->request('POST', "/api/v2/cards/{$cardId}/load", ['amount' => $amountCents, 'currency' => $currency]);
        return ['transactionId' => $r['id'] ?? '', 'amount' => $amountCents / 100, 'status' => 'COMPLETED'];
    }

    public function getCardBalance(string $cardId): array
    {
        $r = $this->request('GET', "/api/v2/cards/{$cardId}/balance");
    ...
    private function request(string $method, string $path, array $body = []): array
    {
        ...
        try {
            ...
            $this->logger->error('Pecunpay API error', ['path' => $path, 'error' => $e->getMessage()]);
            throw new OpenBankingException("Pecunpay error: {$e->getMessage()}", (int) $e->getCode(), $e);
        }
    }
}
